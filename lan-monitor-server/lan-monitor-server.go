package main

import (
	"bytes"
	"encoding/json"
	"flag"
	"fmt"
	"io/ioutil"
	"log"
	"net/http"
	"os"
	"os/exec"
	"path/filepath"
	"strconv"
	"strings"
	"sync"
	"time"

	xj "github.com/basgys/goxml2json"
	"github.com/gorilla/websocket"
)

//VERSION of the program
var version = "undefined-autogenerated"

//ConfigMessage that come from the website/browser and can command this server e.g. changing the NMAP command
type ConfigMessage struct {
	Command      string
	Parameters   string
	Range        string
	ScanInterval uint
	Num          int
}

//StatusMessage that goes from this server to the webclients
type StatusMessage struct {
	ServerStatus uint
	ScanInterval uint
	NMAPStatus   uint
	NMAPError    string
}

//Config data struct to read the config file
type Config struct {
	NMAPRange      string
	NMAPParameters string //comma separated
	HTTPPort       int
	ScanInterval   uint //seconds
}

//ServerStatus summarizes the status of the server
type ServerStatus struct {
	ScanInProgress bool
}

//gobal variable to store all websocket connections for the update
var cons = make(map[*websocket.Conn]bool)

//Data for the nmap scan results
//scanned information
var nmapJSON *bytes.Buffer

//data for the Server Status Message
var statusJSON []byte

//mutex to lock the NMAP JSON information
var lock = sync.RWMutex{}

//Data of the configuration
//the configuration, the string nmap is executed with
var config Config

// mutex to protec the config
var configLock = sync.RWMutex{}

//location of the config file
var configFileLocation string

func echo(conn *websocket.Conn) {
	for {
		m := ConfigMessage{}
		err := conn.ReadJSON(&m)
		if err != nil {
			log.Println("Error reading json.", err)
			break
		}

		//check the content of the message
		//if request or something else all good and proceed
		//if it is a configure message then change the configure

		if m.Command == "configure" {
			//update the current config
			configLock.RLock()
			config.NMAPParameters = m.Parameters
			config.NMAPRange = m.Range
			config.ScanInterval = m.ScanInterval
			log.Println("New re-configuration request from: ", conn.RemoteAddr())
			//save also to disk so that in case of server restart it is remembered
			configJSON, _ := json.Marshal(config)
			err := ioutil.WriteFile(configFileLocation, configJSON, 0644)
			if err != nil {
				log.Println("Can not store the new config - attempt to store in:", configFileLocation)
			}
			log.Println("New configuration [interval:", config.ScanInterval, "sec] is: nmap", config.NMAPParameters, config.NMAPRange)
			configLock.RUnlock()

		}

		lock.RLock()
		if nmapJSON != nil {
			err := conn.WriteMessage(1, nmapJSON.Bytes())
			if err != nil {
				log.Println(err)
				break
			}
		}
		if statusJSON != nil {
			err := conn.WriteMessage(1, statusJSON)
			if err != nil {
				log.Println(err)
				break
			}
		}
		lock.RUnlock()
	}
	delete(cons, conn)
	conn.Close()
}

//ReadConfig reads the config file
func readConfig(configfile string) Config {
	var config Config
	data, err := ioutil.ReadFile(configfile)
	if err != nil {
		log.Println("Error opening for JSON parsing - reverting to defaults")
	}

	err = json.Unmarshal(data, &config)
	if err != nil {
		log.Println("Error parsing json config:", err)
		log.Println("Continuing with defaults", err)
	}

	return config
}

//invokes NMAP
func callNMAP() {
	log.Println("Starting nmap caller")
	var counter = 1
	var scanResultsFileName = "scan.xml"

	for {
		configLock.Lock()
		cmdArgs := make([]string, 0)

		//eliminate the spaces in the parameters
		splitParams := strings.Split(config.NMAPParameters, " ")
		for _, s := range splitParams {
			cmdArgs = append(cmdArgs, s)
		}

		cmdArgs = append(cmdArgs, "-oX")
		cmdArgs = append(cmdArgs, scanResultsFileName)

		//eliminate the spaces in the ranges
		splitRanges := strings.Split(config.NMAPRange, " ")
		for _, s := range splitRanges {
			cmdArgs = append(cmdArgs, s)
		}

		cmd := exec.Command("nmap", cmdArgs...)
		configLock.Unlock()
		log.Println("Init NMAP scan (every", config.ScanInterval, "sec) no:", counter, "as nmap", cmdArgs)
		var stdout bytes.Buffer
		var stderr bytes.Buffer

		cmd.Stdout = &stdout
		cmd.Stderr = &stderr

		//prepare status message
		var statusMsg StatusMessage
		statusMsg.NMAPError = ""
		statusMsg.NMAPStatus = 0
		statusMsg.ServerStatus = 0
		statusMsg.ScanInterval = config.ScanInterval
		statusJSON, _ = json.Marshal(statusMsg)

		//excute the scan
		err := cmd.Run()
		//lock the status and buffers in case somebody else wants to read them in the websockets
		lock.Lock()
		//reset the send buffer in case there is an error
		if nmapJSON != nil {
			nmapJSON.Reset()
		}

		//error evaluation of the nmap cmd.Run
		if err != nil {
			log.Println("Scan failed with -", err)
			statusMsg.NMAPError = stderr.String()
			statusMsg.NMAPStatus = 1
		} else {
			log.Println("Scan no.", counter, "complete")
			counter = counter + 1

			//read the xml file, convert to json and send via the websockets
			xmlfile, err := os.Open(scanResultsFileName)
			if err != nil {
				log.Println(err)
			}
			nmapJSON, err = xj.Convert(xmlfile)
			if err != nil {
				log.Println(err)
			}
		}
		statusJSON, _ = json.Marshal(statusMsg)

		//all status and info can be accessed again
		lock.Unlock()

		log.Println("Writing to websockets")
		for c := range cons {
			if nmapJSON != nil {
				c.WriteMessage(1, nmapJSON.Bytes())
			}
			c.WriteMessage(1, statusJSON)
		}
		<-time.After(time.Duration(config.ScanInterval) * time.Second)
	}
}

func wsHandler(w http.ResponseWriter, r *http.Request) {
	log.Println("Ws request from:", r.Header.Get("Origin"))
	// log.Println("accepted from: http://" + r.Host)
	// if r.Header.Get("Origin") != "http://"+r.Host {
	// 	http.Error(w, "Origin not allowed", 403)
	// 	log.Println("Origin not allowed")
	// 	return
	// }
	conn, err := websocket.Upgrade(w, r, w.Header(), 0, 0)
	if err != nil {
		http.Error(w, "Could not open websocket connection", http.StatusBadRequest)
		log.Println("Upgrade error")
	}

	//add the connection to the pool for broadcast
	cons[conn] = true
	go echo(conn)
}

func main() {
	log.Println("Starting lan-monitor-server ver: " + version)

	//process the config
	//1st the config file is read and set parameters applied
	//2nd the command line parameters are interpreted,
	//if they are set they will overrule the config file
	//3rd if none of the above is applied the program reverts to the hardcoded defaults

	//defaults
	defaultConfigFileLocation := "/etc/lan-monitor.json"
	config.HTTPPort = 8080
	config.NMAPRange = "192.168.10.1/24"
	config.NMAPParameters = "-p 22,80"
	config.ScanInterval = 120 //seconds

	displayVersion := flag.Bool("version", false, "Prints the version number")
	createExampleConfig := flag.Bool("config", false, "Writes the lan-monitor.json to local dir as example")
	cmdlineHTTPPort := flag.Int("port", config.HTTPPort, "HTTP port for the webserver")
	cmdlineNMAPScanRange := flag.String("range", config.NMAPRange, "The range NMAP should scan e.g. 192.168.1.1/24 it has to be nmap compatible")
	cmdlineScanInterval := flag.Uint("scan-rate", config.ScanInterval, "The interval of the scans in seconds")
	flag.StringVar(&configFileLocation, "config-file", defaultConfigFileLocation, "Location of the config file")
	cmdlineScanParameters := flag.String("scan-parameters", config.NMAPParameters, "The ports that will be scanned")
	flag.Parse()

	//try to read the configfile
	_, err := os.Stat(configFileLocation)
	if err == nil {
		config = readConfig(configFileLocation)
	} else {
		log.Println("Config file is missing - looked at:", configFileLocation)
		log.Println("Reverting to commandline/defaults")
	}

	//if no range is defined in the config file
	if config.NMAPRange == "" {
		config.NMAPRange = *cmdlineNMAPScanRange
	}

	//if no port is defined in the config file
	if config.HTTPPort == 0 {
		config.HTTPPort = *cmdlineHTTPPort
	}

	//if no scan interval is defined in the config file
	if config.ScanInterval == 0 {
		config.ScanInterval = *cmdlineScanInterval
	}

	//if no ports to be scanned are defined
	if config.NMAPParameters == "" {
		config.NMAPParameters = *cmdlineScanParameters
	}

	log.Println("Config - range:", config.NMAPRange, "webserver port:", config.HTTPPort, "interval:", config.ScanInterval, "sec", "scan-parameters", config.NMAPParameters)

	if *displayVersion == true {
		fmt.Println("Version: " + version)
		return
	}

	if *createExampleConfig == true {
		workingDir, _ := os.Getwd()
		fmt.Println("Config in JSON format written to:", workingDir)
		configJSON, _ := json.Marshal(config)
		err := ioutil.WriteFile("lan-monitor.json", configJSON, 0644)
		if err != nil {
			fmt.Println("Error writing demo config file")
		}
		return
	}

	workingDir, _ := os.Getwd()
	log.Println("Working Dir:" + workingDir)

	httpDir := filepath.Dir(workingDir) + "/www"
	//init the scanning routine
	go callNMAP()

	//starting and configuring the webserver
	fs := http.FileServer(http.Dir(httpDir))
	http.HandleFunc("/ws", wsHandler)
	http.Handle("/", fs)
	listenAddress := ":" + strconv.Itoa(config.HTTPPort)
	log.Fatal(http.ListenAndServe(listenAddress, nil))
}
